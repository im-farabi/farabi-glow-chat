...
/**
 * Website Generator API Functions
 */
...
export async function validateWebsiteCode(
  html: string, 
  css: string, 
  js: string
): Promise<{ valid: boolean; errors: string[]; warnings: string[] }> {
  try {
    const { data, error } = await supabase.functions.invoke('validate-website', {
      body: { html, css, js }
    });
    ...
    return { valid: false, errors: ['Failed to validate code'], warnings: [] };
  }
}

export async function publishWebsite(websiteData: {
  ...
  try {
    ...
    throw new Error(error.message || 'Failed to publish website');
  }
}

export async function getUserWebsites(anonymousUserId: string): Promise<any[]> {
  try {
    const { data, error } = await supabase.functions.invoke('get-user-websites', {
